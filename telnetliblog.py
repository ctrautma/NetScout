...
class Telnet2(object):
    ...
    def __init__(self, host=None, port=0,
                 timeout=socket._GLOBAL_DEFAULT_TIMEOUT, telnet_log=None):
        """Constructor.
        ...
        self._tlnt_log = logging.Logger('app')
        handler = telnet_log if telnet_log else RotatingFileHandler(
            'telnet.log', mode='a', maxBytes=8*1024*1024,
            encoding=locale.getlocale()[1])
        handler.setLevel(logging.INFO)
        self._tlnt_log.addHandler(handler)
        self._tlnt_log.info('#' * 80)
        self._tlnt_log.info('\r\rLog started -- {} --'.format(time.asctime()))
